...
void	pdr_recv_g_s(graph_t *graph, t_player *player, int type)
{
	switch (type) {
	case 6:
...
void	pdr_recv_g_ff(graph_t *graph, t_player *player, int type)
{
	switch (type) {
	case 4:
	...
	case 5:
		...
		break;
	default:
		pdr_recv_g_s(graph, player, type);
		break;
	}
}

void	pdr_recv_g_tt(graph_t *graph, t_player *player, int type)
{
	switch (type) {
	case 2:
	...
	case 3:
		...
		break;
	default:
		pdr_recv_g_ff(graph, player, type);
		break;
	}
}
...
void	pdr_recv_g(graph_t *graph, char **args)
{
	...
	case 1:
		...
		break;
	default:
		pdr_recv_g_tt(graph, player, atoi(args[2]));
		break;
	}
}